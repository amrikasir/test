					...
					<form method="post" action="{{ route('post.permintaan') }}" id="new.request">
						...
						<div class="flex flex-wrap">
							<div class="w-full lg:w-6/12 px-4">
								<div class="relative w-full mb-3" x-data="{nik: ''}">
									<label class="flex justify-between block uppercase text-blueGray-600 text-xs font-bold mb-2"
										htmlfor="grid-password">
										NIK Peminta
										<span x-text="nik.length">16/16</span>
									</label>
									<input type="text" x-model="nik"
										...
										<p x-show="message != '' || message != null" x-text="message" class="text-sm pt-2 text-red-600/75"></p>
								</div>
							</div>
							<div class="w-full lg:w-6/12 px-4">
							...
							<div class="w-full lg:w-6/12 px-4">
								<div class="relative w-full mb-3">
									...
									<input type="text" class="border-0 px-3 py-3 placeholder-blueGray-300 text-blueGray-600 bg-white rounded text-sm shadow
										...
										disabled:bg-gray-100" placeholder="-" x-model="nama" disabled>
								</div>
							</div>
							<div class="w-full lg:w-6/12 px-4">
								<div class="relative w-full mb-3">
									...
									<input type="text" class="border-0 px-3 py-3 placeholder-blueGray-300 text-blueGray-600 bg-white rounded text-sm shadow
										focus:outline-none focus:ring w-full ease-linear transition-all duration-150
										disabled:bg-gray-100" placeholder="-" x-model="depart" disabled>
								</div>
							</div>
						</div>
							...
							<div class="w-full px-4">
								<div class="relative w-full mb-3">
									...
									<div class="ui fluid multiple search selection dropdown">
										<input require type="hidden" id="selected" x-model="selected">
												...
												<tr>
													...
													<td class="center aligned">
														<template x-if="item.stok == 0">
															<span><i class="icon red delete"></i> Kosong</span>
														</template>

														<template x-if="item.qty > 0 && item.qty <= item.stok">
															<span><i class="icon green checkmark"></i> Terpenuhi</span>
														</template>

														<template x-if="item.qty > 0 &&  item.qty > item.stok">
															<span><i class="icon yellow warning"></i> Sebagian</span>
														</template>
													</td>
												</tr>
											</template>
